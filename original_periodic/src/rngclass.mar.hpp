...
//    statestr << std::setprecision( std::numeric_limits<double>::max() );
  ...
  friend void serialize(const rngclass& rng, std::ostream& stream);
  friend void deserialize(rngclass& rng, std::istream& stream);
};
void serialize(const rngclass& rng, std::ostream& stream){
  // make sure ostream was opened with std::ios::binary!
  dlib::serialize(rng.seed, stream);
...
void deserialize(rngclass& rng, std::istream& stream){
  // make sure istream was opened with std::ios::binary!
  dlib::deserialize(rng.seed, stream);
