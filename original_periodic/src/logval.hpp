...
void serialize(const logval& x, std::ostream& out)
{
    dlib::serialize(x.lnx, out);
}

void deserialize(logval& x, std::istream& in)
{
    dlib::deserialize(x.lnx, in);
}
...
inline double logval::toDouble() const { return exp(lnx); }
...
inline logval& logval::addLarger(logval larger) {
	lnx = larger.lnx + log1p(exp(lnx - larger.lnx));
	return *this;
}
